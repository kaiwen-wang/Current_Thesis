...
\title{On Frontend Frameworks, Design Systems, and AI with Commentary on Political and Technological Means}
\author{Kaiwen Wang}
...
This thesis is about technology, or more specifically, frontend design systems and component libraries in Vue with discussion on the development of technologies, its relation to politics, and the effects of Large Language Models (LLMs), colloquially referred to as AI.

, but we start with an analogy to set the pace.
...
Currently we live in an age where technology is subject to the whims of geopolitics and perhaps this shall grow even more in the coming years. Though this has always been perennially true, the recent decades of unipolar hegemonic peace among nations is thawing. Power politics is in bloom, and for that we must consider what world or ecosystem to be a part of.

It is easy for one to say: learn this, learn that. But doing so is an entry-point into patterns of interlocking norms, systems, tools, conferences, words and mannerisms, and other characteristics of one world distinct from another. Should one enter the world of React, they may find "useEffect," "useState" to be a common pattern. Conferences may revolve around specific geographies and locations and have similar sponsors. The same people and faces pop up.

Vercel has gradually acquired much of the ecosystem around React, including the acquisition of Svelte and Nuxt. Whereas Svelte 4 previously declared variables with a simple "let x = 1," Svelte 5 after the acquisition now is similar to React with "let x = useState(1)." The acquisiton of Nuxt, I believe, is meant to implicitly align conventions of Vue with that of React downstream of Vue itself, as attempts to acquire Vue likely stalled.

Despite the claims that this is meant for open source, political realists know it is actions and presentations rather than words which determine how one is perceived. I believe that the maintainers of React possess an ethnic chauvinism as indicated by believing English as a language that stands alone amongst all and Guillermo Rauch's endorsement of Benjamin Netanyahu. The religion of the leader, generally, is the religion of the land and the subordinates.

By failing to create a universal set of standards predicated upon the common unity of man such as open-source software does but instead one centered around one social-demographic group, the splintering of the frontend world into blocs remains a constant point of consideration. In some ways this writing is contributing to it. The sensitivity of the rest of the world of "fifth generational warfare" and the role that the internet, the browser, frontend web frameworks, and design has in shaping the attitudes and beliefs of the masses means that the politicization of React has heightened political and social consequences.
...
Since frontend frameworks are one level above the browser, the geopolitical risk must be considered as per fields such as semiconductors. The nature of hardware and software presents different tendencies for dealing with it: hardware can be embargoed and is harder to replicate. Software has similarities to finance: its strength is in its network ties and the community ecosystem/development momentum.

Though I had initially thought the VS Code AI Agent extension "Cline" to be supplanted by other competitors such as Roo Code or Kilo Code because anyone could fork it, the enduring advantages of Cline come from its firstcomer brand name, the developers' familiarity with the codebase, and the legitimacy that allows them to partner with AI labs to serve models through Cline. Even if one were to make the best software or models, without a willing base to transact and integrate with them into parts of the network, they would not be financially successful. If a company were to acquire the assets, it would acquire the original rather than a fork, though at times large companies do rip off open source projects such as Amazon with Elasticsearch for their own being.

Thus despite someone making the highest quality product at the lowest prices, integration into a system or network is not guaranteed. The only alternative is to integrate into networks which value such things, trimming back other networks as higher efficiencies over time lead to growth of what is higher quality.

These same enduring advantages that Cline has are in similar fashion likely to accrue to Chromium in the event of a fork or split. Any fork would maintain viability for the current frameworks up to today but a fragmentation risk for Vue is likely to emerge should it be required to choose between two sets of browser standards. The fork in most cases would lag behind, delivering a mostly acceptable but not leading edge experience, or it may in some rare cases with enough firepower behind it leap ahead of the original copy.
...
Quasar aesthetically seems very similar to ElementPlus. It is messy, unstructed, and contains much extraneous information. Vue components are just one of many sections, as it seems to want to build for multiple devices and apps at the same time rather being just a component library.
...
The page loads but the design is somewhat tacky. A moving walkway of pictures appears on the right but the animation ease is linear giving it a boring feel. The spacing of the pictures relative to the screen appears not so good. Like other design systems, there a bit too many extra unnecessary sections. The component categorization is different compared to the other libraries.
...
\section{A review of the above design systems}
...
\begin{table}[ht]
    \centering
    \caption{Comparison of Component Categories Across Vue Design Systems}
    \label{tab:component-categories}
    % Updated command to take 1 argument explicitly
    \newcommand*\rot[1]{\rotatebox{90}{#1}}

    \begin{tabular}{@{}lccccccc@{}}
        \toprule
        \textbf{Category}    & \rot{PrimeVue} & \rot{ElementPlus} & \rot{Naive UI} & \rot{Ant Design Vue} & \rot{Arco Design} & \rot{TDesign} & \rot{Flowbite Vue 3} \\
        \midrule
        % Core & Foundational
        General / Basic      &                & $\bullet$         & $\bullet$      & $\bullet$            & $\bullet$         & $\bullet$     &                      \\
        Layout               &                &                   & $\bullet$      & $\bullet$            & $\bullet$         & $\bullet$     &                      \\
        Navigation / Menu    & $\bullet$      & $\bullet$         & $\bullet$      & $\bullet$            & $\bullet$         & $\bullet$     &                      \\
        \midrule
        % Data Interaction
        Data Entry / Form    & $\bullet$      & $\bullet$         & $\bullet$      & $\bullet$            & $\bullet$         & $\bullet$     & $\bullet$            \\
        Data Display         & $\bullet$      & $\bullet$         & $\bullet$      & $\bullet$            & $\bullet$         & $\bullet$     &                      \\
        \midrule
        % User Feedback
        Feedback / Messages  & $\bullet$      & $\bullet$         &                & $\bullet$            & $\bullet$         & $\bullet$     &                      \\
        \midrule
        % Specific / Granular
        Button               & $\bullet$      &                   &                &                      &                   &               &                      \\
        Panel                & $\bullet$      &                   &                &                      &                   &               &                      \\
        ...
        \midrule
        % Meta & Utility
        Configuration/Config &                & $\bullet$         & $\bullet$      &                      &                   & $\bullet$     &                      \\
        Utility / Utils      &                &                   & $\bullet$      &                      &                   & $\bullet$     & $\bullet$            \\
...
\section{Analysis of Component Categories}

\subsection{Repeated and Common Categories}
...
\begin{itemize}
    \item \texttt{PrimeVue} has the most granular categorization, with many groups that other libraries merge into broader categories. \texttt{Panel}, \texttt{Overlay}, \texttt{File}, \texttt{Chart}, \texttt{Media}, and \texttt{Button} are all separated into their own top-level categories, which is unique among this set.
...
\section{When theming Vue components, some are hospital software and some is not}

% styles table, styles typography, styles etc. ignore the ones I think are bad


\section{In-depth comparison of frequency of all component types}
