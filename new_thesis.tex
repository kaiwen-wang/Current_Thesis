...
\begin{itemize}
    \item \texttt{PrimeVue} has the most granular categorization, with many groups that other libraries merge into broader categories. \texttt{Panel}, \texttt{Overlay}, \texttt{File}, \texttt{Chart}, \texttt{Media}, and \texttt{Button} are all separated into their own top-level categories, which is unique among this set. \texttt{Reka UI} takes the opposite approach, grouping things into only 3 top-level categories. Some such as \texttt{shadcn-vue} only sort components alphabetically, having no cateoegires at all.
