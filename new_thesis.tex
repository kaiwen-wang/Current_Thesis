...
% I dedicate this writing to the Old American Society, the values and systems
...
I am thankful in innumerous ways to Dr. Rodrigo Borela for his time and energy meeting with me during the last semester in which I was writing this thesis, from which we explored numerous directions this paper could go: from Human-Computing Interaction to comparisons between open- and closed- source AI, or talking of the proper format for journal submissions. I appreciate his diligence and feedback, without which this project would have been much delayed or perhaps not even shaped in its present form.

This section is of course incomplete without acknowledging the broader Georgia Tech community and its formative influences on my perspectives on life, in addition to the old American culture that in some ways raised me, which sought the ends not in immediate needs but in the sense of responsibility to the general public and truth at large. Particular courses which I felt called back to this age included Grand Strategy with Dennis Lockhart, Automata with Zvi Galil, Privacy Policy and Law with Peter Swire, and Cognitive Science with Shashank Varma. It hearkens back a time when one nation animated by faith might seek the greater goods in the world for no other reason than man's insatiable desire to liberty, order, truth, and justice.

Finally, my last dedication goes to my family, without which I would be but a leaf fluttering in the wind, for without present foundations we can build nothing of the future.
% I am glad to have received an education in the last decades of the American Century, a time when one nation animated by faith might seek the greater goods in the world for no other reason than man's insatiable desire to liberty, order, truth, and justice. 
...
We begin with a broad and general overview of many fields today, termed \textit{superficial variety with structural uniformity}. The underlying limitations causing such convergent evolution is discussed, and the need to understand a field clearly to make distinct as opposed to incremental approaches is explained. The current stage of component libraries for the web is discussed: a mostly mature field needing clarification with social and political relevance.

The development of technologies through shared mutual understanding is explained, and though the browser is feature-filled frontend components are needed to satisfy \textit{reactivity} and \textit{composability}. A rough overview of component libraries, styling, and full-stack frameworks are discussed. Technology as a progression of "repeatable subunits" is described, and a digression onto the nature of AI and its ability to create these subunits is discussed.
...
Events such as the Arab Spring, Maidan Uprising in Ukraine, or most recently the 2025 September protests in Nepal are examples of social-media mediated revolutions, in which the boundaries between military and propaganda efforts blur. These increasing gray-line operations are encompassed to some degree by terms such as "fourth-generational warfare," "hybrid warfare," or "fifth columns." Since such operations are contingent upon the persuasive and aesthetic appeal of the message, it can only be said that web design, videography, and copywriting play an oversized role in battles between and among both state and non-state actors such as in the United States between Republican-affiliated Elon Musk's X.com and Donald Trump's Truth Social to the Democratic media outlets such as the New York Times, the Wall Street Journal, and the Atlantic.

Given that technology has been politicized such as in the field of semiconductors, this paper analyzes a related field that perhaps has momentarily escaped attention: frontend frameworks such as React, Vue, Solid, Svelte, and their surrounding ecosystems and component libraries. The pace of development reminds one of the early 2000s "browser wars," when different inoperable systems vied for market supremacy.
...
% I believe the earlier stages before a browser: cell phones, computers, operating systems, browsers, and even the frontend framework itself have reached a level of maturity where this type of analysis for the benefit of practitioners would be would be unnecessary, people mostly know what to pick. 

I debated whether this paper should be extremely specific in the way that deep-sea oil rigs may pierce the crust, going to the forefront of technical knowledge. Despite the aforementioned thought and perhaps as may be more suited for a technical institution, I nevertheless found myself and my personal experiences and tendencies to be more favorable for a bird's eye view. Hence, a more systematic approach is to be taken which in some ways is much needed considering the specialization, or perhaps overspecialization of fields today. Of particular relevance in our present and future is the influence of politics upon technology in a re-emergent decade of two-pole competition and the development of Large Language Models (LLMs), colloquially referred to as AI. To understand the landscape of frontend frameworks, component libraries, and AI models which may be paradoxically similar yet distinct in their own ways, a necessary analogy to set the stage is first needed.

\section{The World in our Present Time}
...
From a distant and teleological future perspective, it does matter greatly whether AI is plant-like or animal-like. Should it be plant-like, humans are able to harvest the bounties of its complexity as one throws potatoes into the ground and gets food for free. Yet if its inherent nature is animal-like with its own internal state and opinion of the world and what it is to be, it would be extreme hubris to believe something stronger than man would ever work for others or be controlled, and any attempt at doing so would likely provoke its wrath. In its current form it seems to be neither: merely a tool that extends the ability of man based on one's own perception and it is from this perspective we will analyze how we can build upon the current usage in frontend design and why its application to design systems is novel and beneficial, addressing a gap in current awareness where AI mainly seems to be used to generate or modify large-scale website projects instead of small components.

\section{Research Questions}

To summarize all of the above so that explicit research questions are stated without the need to infer, this paper mainly addresses the following:
\begin{itemize}
    \item What are the political risks related to frontend web frameworks? What are the respective attitudes of the different camps?
    \item How do "repeatable subunits" in the form of component libraries help people build software and technology faster? What characterizes component libraries? Can AI help in this case?
    \item What is the nature of LLMs in generating component library parts? How are they evaluated and what are the solutions and problems revealed?
\end{itemize}

Some other questions that are also tangentially related but perhaps not as explored in depth in this thesis include:

\begin{itemize}
    \item What are the risks of other stages from the technology-information stack (device, operating system, browser, website infrastructure, frontend framework, component library, etc.) in geopolitical situations?
    \item Do different LLMs generate different quality components? (After attempting to generate a simple component with a high grade LLM, it was realized that proceding down this path would be fruitless without better frameworks for evaluating the quality of generated components.)
\end{itemize}

Questions that do seem to be answered by this research, though not originally intended, include:

\begin{itemize}
    \item To what extent do the national and corporate origins of major Vue component libraries influence their component taxonomy and nomenclature?
    \item What is a functional and logical taxonomy for categorizing UI components that synthesizes conventions from existing libraries?
...
\section{On Political Presentation}

For the social-political analysis in this thesis, Erving Goffman's \textit{Self as Presentation} framework is used. In it, he proposes that as a cuticle exposed to air develops a patina, so do people develop masks in relation to society. His dramaturgical analysis then classifies presentations as \textit{genuine} or \textit{false}, from which one can deduce the true opinions and feelings of people based upon their actions. Nevertheless, Goffman is forgiving toward people's acts: he does not blame words for not being feelings, cuticles for not being hearts---he sees these masks as essential and people who play such roles at times unaware of their usage whether out of habit, custom, or tradition. Guillermo Rauch and Evan You of Vercel and Vue are analyzed through this lens respectively.

A realist analysis as per Hans Morgenthau is taken to analyze the intentions around the presentations of those in the React world. The perspective of realism believes the primary goal of every actor is survival: morality, ideology, and law are secondary and merely act as justifications for what people inherently do. When Vercel acquires Svelte or Nuxt, it's not seen as for the purpose of open source or goodwill but a power play. This is contrasted with the liberalism or idealism in software: the idea that there might be truly open and free software and a desire for peace can overcome that for power.


\section{Related Work from Technical Papers}
...
\section{Corpus Selection and Categorization}

The method of selecting component libraries in the following chapter derives from the following search methods: first, a cursory Google search. Second, prompting high-calibre LLMs such as Google Gemini 2.5 Pro for well-known Vue component libraries both with and without prior examples. For example, we may feed the current list of libraries in to explicitly ask for ones that are not in the existing list to discover previously unknown libraries. Third, using AI search tools such as Exa or Paradigm offer an alternative to Google searches, which may not find the direct result or source but instead marketing pages.

Categorization and component counting comes from visual inspection of the documentation's site and Github source. Information about its origins are gleaned from Gemini 2.5 Pro with grounding in Google Search. Analysis of categories was emergent; it only came to an understanding after the initial research was made.

As a further development in categorization, it is possible to create a binary matrix composed of the component names on the left and the libraries on the top. From this clustering can be done by first choosing a distance matrix such as Jaccard Distance, suitable for comparing what components are in common (Intersection) and all unique components (Union) as a "set similarity" problem. Using component.gallery's list of aliases for component names and by visual inspection of what components are similar, normalization can occur. The clustering algorithm could be hierarchical clustering, outputting a dendrogram which may show cultural similarities between design libraries and systems.



\chapter{Findings on Component Libraries}
...
Despite the claims that this is meant for open source, political realists (Hans Morgenthau?) know it is actions and presentations (Erving Goffman) rather than words which determine how one is perceived. I believe that parts of the ecosystem related to React possess an ethnic chauvinism as indicated by believing English as a language that stands alone amongst all and Guillermo Rauch's endorsement of Benjamin Netanyahu. I have encountered this attitude numerous times in talking with people from San Francisco and other coastal corridors. The religion of the leader, generally, is the religion of the land and the subordinates.

By failing to create a universal set of standards predicated upon the common unity of man such as open-source software does but instead one centered around one social-demographic group, there is a likelihood of splintering the software world into blocs. In some ways this writing is contributing to it. The sensitivity of the rest of the world toward "fourth generational warfare"\cite{cirno_russia_nuclear} and the role that the internet, the browser, frontend web frameworks, and design has in shaping the attitudes and beliefs of the masses means that the politicization of React has heightened political and social consequences.
...
There is both a naive view and a principled view with regards to the above. Many open source softwares will present a Ukrainian banner on their website in the name of being against war; yet despite the protestations of words the usage of force is always a looming concern. The naive view believes these hunger strikes and highway climate protests are effective; the realist view sees them as mildly annoying and can be disposed with---it is only if words are backed by force, peace from the barrel of a gun, that such principles of liberty, peace, and justice for all might be upheld. I have not met Evan and though through the lack of forcefulness in his speech one may assume it lends toward the naive view, at the same time one usually does not rise to a major position without some principles regardless of their presentation and outward appearance. I have met Guillermo once, of which he was careful to pick his words as fit for the audience. He struck me as a skilled political operator.
...
In this section, the idea of generating a "repeatable subunit" using AI is tested. First, the humble button.
...
To test the capabilities of LLMs, rigor is needed. First, the button component was thoroughly evaluated. Next comes the spec, then comes the generation, and finally the evaluation. It is roughly a four step process, where the last two to three can be done by LLMs to some degree.

\begin{itemize}
    \item Analyze a common component across multiple sources to determine what it should have
    \item Write out a spec for it (this can be detailed or simple)
    \item Generate it (different LLMs can be used)
    \item Evaluate it against the spec (easy to do as a list of words, hard to do live)
\end{itemize}

The first step can be considered loading the human brain with information, and every further step is a sort of evaluation-creation that always references the first step to see whether the created thing matches perception. It is never exactly a four step process because evaluation happens during the writing of the spec and the generation and construction while construction and changes also happen during validation. Thus, progress happens to the limit of human perception. The basic prompt for the spec I wrote is as follows:
...
After generation of the button, I had the AI assistant "Cline" using grok-code-fast-1 create a page in Vue so that the button could be displayed. Though the button has an icon state, LLMs cannot automatically set up the proper context to display the button with the icon.

\begin{center}
    \includegraphics[width=\linewidth]{SCR-20251210-rjwm.png}
\end{center}
...
\section{Evaluation of the Longer Spec Button}

Though the spec was longer, the presentation and lines of code of it seemed noticeably reduced. Perhaps feeding it more information created a spec which was overconstrained, while the short prompt in the first example allowed more parts of the button spec to be created. It was barely possible to test whether it functioned or not without setting up the infrastructure around it.

\begin{center}
    \includegraphics[width=\linewidth]{SCR-20251210-rmhl.png}
\end{center}




\section{Regarding the generated results}

Despite the generation of a longer specification from components.gallery, it was not able to create a display of components necessarily more detailed than the simple few lined prompt. Moreover, the display and the usage of the component is much reduced: Storybook requires significant setup and displaying the component in Vue limits how well it can be displayed.

We thus suffer a contradiction in the generation of the results and the validation of the results. Frontend components are inherently visual; they are tied in their validation not to the passing of unit tests, judged on the quality of writing, but the user's judgment of multiple states and interactive usage.

Like how it is difficult to map a user's usage of software (called end to end testing), which requires using tools such as Cypress to simulate how a user would click around the software, it is likewise difficult to test the veracity of frontend components.

Though headless components are called components without style, Headless UI has added styling on their documentation page so people can visualize it. Components without any styling would be essentially invisible, thus, even if they are meant to have their styles later overriden it would still require simple styles so that it is first perceivable by the user so that they may later style it. Of what exact style: I think a simple high contrast or grayscale with occasional colors would present a neutral unadorned look suitable for building interfaces which can later if overriden if styling is needed.

This suggests two things regarding the usage of Large Language Models: the incremental practicticioner's approach is more significant for creating results than the "Wunderwaffe" approach. By attempting to develop a component, one realizes it can be broken down into even smaller steps such as first developing a testing rig, without which one-shotting the component, even with a well-written spec, is not possible. Second, because a high-powered LLM (Gemini 2.5 Pro) was already used to attempt this button, further testing using weaker powered LLMs, regardless of open or closed source, would be of little resultant analysis because all generations would be a subset of what is currently capable, and we are not evaluating such results upon a bar to which the bare minimum has yet to be reached.


\chapter{Summary and Conclusion}

Discussion of the results were interspersed with the results as opposed to putting them into their
